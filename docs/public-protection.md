...
- Root text/hashtag searches require verification, with 60/minute per verified browser and 1200/minute aggregate. Verification submissions have a separate key at 60/minute per IP.
...
Deployment requires TURNSTILE_SECRET_KEY and the four ABUSE_*_LIMIT bindings in wrangler.jsonc. The real interactive Turnstile flow must also be checked in a browser; automated tests mock the Turnstile response.
