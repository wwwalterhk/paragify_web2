...
type GenerateHeading2ImgReadyPostRow = {
	post_id: number;
	user_pk: number;
...
export async function GET(request: Request) {
	try {
		...
		const generateHeading2ImgRaw = readString(requestUrl.searchParams.get("generate_heading_2_img"));
		const generateHeading2Img = toOptionalInteger(generateHeading2ImgRaw);
		if (generateHeading2ImgRaw !== null && generateHeading2Img === null) {
			return NextResponse.json({ ok: false, message: "generate_heading_2_img must be an integer" }, { status: 400 });
		}
		const generateHeading2ImgFilter = generateHeading2Img ?? 1;

		const whereClause = `
      p.visibility = 'prepare'
      AND p.prepare_content IS NOT NULL
      AND trim(p.prepare_content) <> ''
      AND p.generate_heading_2_img = ?
    `;
		const whereBindings: Array<number> = [generateHeading2ImgFilter];
		...
		const postsResult = await db
          ...
          LIMIT ? OFFSET ?`,
			...
			.all<GenerateHeading2ImgReadyPostRow>();
		...
		return NextResponse.json({
			...
			filters: {
				...
				generate_heading_2_img: generateHeading2ImgFilter,
			},
		});
	} catch (error) {
		return NextResponse.json(
			{
				ok: false,
				message: error instanceof Error ? error.message : "Failed to load generate-heading-2-img posts",
			},
			{ status: 500 },
		);
	}
