...
async function handleCoverImgUpdate(request: Request) {
	try {
		...
		const hasGenerateCoverImg =
			body !== null && typeof body === "object" && Object.prototype.hasOwnProperty.call(body, "generate_cover_img");
		const parsedGenerateCoverImg = hasGenerateCoverImg ? parseBodyInteger(body?.generate_cover_img) : 0;
		...
		if (hasGenerateCoverImg && parsedGenerateCoverImg === null) {
			return NextResponse.json({ ok: false, message: "generate_cover_img must be an integer when provided" }, { status: 400 });
		}
		const generateCoverImg = parsedGenerateCoverImg ?? 0;
...
export async function POST(request: Request) {
	return handleCoverImgUpdate(request);
}
