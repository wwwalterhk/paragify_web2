...
export async function POST(req: Request) {
	try {
		...
		const reportTableAvailable = await hasTable(db, "post_reports");
		if (!reportTableAvailable) {
			return NextResponse.json({ ok: false, message: "post_reports table unavailable" }, { status: 500 });
		}
		...
		const body = (await req.json().catch(() => null)) as
			| { postId?: number; reason?: string; details?: string }
			| null;
		...
		if (!reason) {
			return NextResponse.json({ ok: false, message: "reason required" }, { status: 400 });
		}

		await db
			.prepare(
				"INSERT OR IGNORE INTO post_reports (post_id, reporter_pk, reason, details, status) VALUES (?, ?, ?, ?, 'open')",
			)
			.bind(postId, userPk, reason, details)
			.run();

		await db
			.prepare(
				"UPDATE post_reports SET reason = ?, details = ?, status = 'open', resolved_at = NULL WHERE post_id = ? AND reporter_pk = ?",
			)
			.bind(reason, details, postId, userPk)
			.run();

		const report = await db
			.prepare(
				`SELECT report_id, post_id, reporter_pk, reason, details, status, created_at, reviewed_by, review_note, resolved_at
         FROM post_reports
         WHERE post_id = ? AND reporter_pk = ?
         LIMIT 1`,
			)
			.bind(postId, userPk)
			.first();
		...
		return NextResponse.json(
			{ ok: false, message: error instanceof Error ? error.message : "Failed to report post" },
			{ status: 500 },
		);
	}
}
