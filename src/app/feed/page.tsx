...
function getFeedSeoContext(activeTag: string | null, activeUserId: string | null): {
	title: string;
	description: string;
	...
	if (activeTag && activeUserId) {
		return {
			title: `@${activeUserId} posts with #${activeTag} | ${SITE_NAME} Feed`,
	...
	if (activeTag) {
		return {
			title: `#${activeTag} posts | ${SITE_NAME} Feed`,
	...
	if (activeUserId) {
		return {
			title: `@${activeUserId} posts | ${SITE_NAME} Feed`,
...
export async function generateMetadata({ searchParams }: FeedPageProps): Promise<Metadata> {
	...
	const seoContext = getFeedSeoContext(activeTag, activeUserId);
...
export default async function FeedPage({ searchParams }: FeedPageProps) {
	...
	const sessionUser = session?.user as
		...
		| undefined;
	const { posts, hasMore, nextCursor, error } = await getInitialFeed(sessionUser?.email ?? null, activeTag, activeUserId);
	const seoContext = getFeedSeoContext(activeTag, activeUserId);
	const canonicalUrl = toAbsoluteUrl(seoContext.canonicalPath);
	...
	const websiteJsonLd = {
		...
		potentialAction: {
			"@type": "SearchAction",
			target: `${SITE_URL}/feed?tag={search_term_string}`,
	...
	return (
		<main className="min-h-screen pb-12 text-[color:var(--txt-1)]">
			...
			<div className="mx-auto w-full max-w-xl px-3 pt-6">
				{error ? (
					<p className="mb-5 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">{error}</p>
				) : null}
